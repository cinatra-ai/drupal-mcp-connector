...
- **`server-only` → `dependencies`, `^0.0.1`.** Imported for its side effect (throws if a server-only module lands in a client bundle) in three production files: `src/lib/drupal-mcp-client.ts`, `src/mcp/toolbox.ts`, `src/settings-page.tsx` — the first two ship in the published `files` payload alongside `settings-page.tsx`. `dependencies`, not `devDependencies`: it is a production import, not a test/build-time-only one. `^0.0.1` matches the cinatra host's own range (there has only ever been one `0.0.1` release, so the caret is effectively an exact pin either way).
