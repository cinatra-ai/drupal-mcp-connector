...
That rule is now fully satisfied: every third-party import this repo has (`@modelcontextprotocol/client`, `zod`, `server-only`, `lucide-react`, `next`, `vitest`) is declared in this package's own `package.json`. Do not assume a future import resolving today means it is declared — check, and add it here rather than letting it ride the host's hoist.
